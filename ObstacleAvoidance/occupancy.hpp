...
namespace ocpncy {
	...
	*/
	...
	class bmap_item_iterator {
		...
		public:
			void reset() {
				if (item.depth) parents[0] = reinterpret_cast<btile_tree*>(item.ptr);
					...
					if (branch_indices[current_level] > 3) branch_indices[--current_level]++;
						...
						if (parents[current_level]->branch[branch_indices[current_level]]) {
							...
							if (current_depth == 1) {
								...
								parents[current_level + 1] = reinterpret_cast<btile_tree*>(parents[current_level]->branch[branch_indices[current_level]]);
	...
	void PrintItem(const bmap_item<log2_w>& item) {
		if (item.depth == 0) {
			PrintTile(*static_cast<btile<log2_w>*>(item.ptr));
		}
		...
		while (tile = iterator.next())
			for (int y = 0; y < (1 << log2_w); y++)
				for (int x = 0; x < (1 << log2_w); x++)
					if (get_bit(x, y, tile)) img[y + iterator.last_origin().y][2 * (x + iterator.last_origin().x)] = '@';
		*/
		...
		unsigned int current_depth;
		while (branch_indices[0] < 4) {
			std::cout << "Current Depth: " << (item.depth - current_level) << std::endl; //test
			std::cout << "Branch Index: " << branch_indices[current_level] << std::endl; //test
			if (branch_indices[current_level] > 3) branch_indices[--current_level]++;
			else {
				if (parents[current_level]->branch[branch_indices[current_level]]) {
					current_depth = item.depth - current_level;
					...
					if (current_depth == 1) {
						...
						std::cout << "Tree will be processed next" << std::endl; //test
						parents[current_level + 1] = reinterpret_cast<btile_tree*>(parents[current_level]->branch[branch_indices[current_level]]);
						branch_indices[current_level + 1] = 0;
						current_level++;
					}
			...
			if (branch_indices[current_level] > 3) branch_indices[--current_level]++;
				...
				if (src_parents[current_level]->branch[branch_indices[current_level]]) {
					...
					if (current_depth == 1) {
						...
						src_parents[current_level + 1] = reinterpret_cast<btile_tree*>(src_parents[current_level]->branch[branch_indices[current_level]]);
						current_level++;
					}
				}
				else branch_indices[current_level]++;
	...
	*/
	...
	class bmap_fstream : bmap_iostream<log2_w> {
		protected:
			...
			*/
			item_index indexed_item_at_depth(const item_index& start, const gmtry2i::vector2i& p, unsigned int gdepth) {
				item_index item;
			...
			* Returns a real item if start is real, although it won't contain the point if start doesn't contain the point
			*/
			item_index seek_item_at_depth(const item_index& start, const gmtry2i::vector2i& p, unsigned int gdepth) {
				item_index item;
				item_index next_item = indexed_item_at_depth(start, p, gdepth);
				unsigned int next_branch_idx;
				unsigned int hwidth = 1 << (next_item.depth + log2_w - 1);
			...
			/*
			* Always successfully writes the tile, but it will be in the wrong place if the map doesn't contain the point
			*/
			void write_tile_at_bottom(const item_index& start, const gmtry2i::vector2i p, const btile<log2_w>* tile) {
				item_index item = seek_item_at_depth(start, p, 0);
				if (item.depth == 0) {
			...
			void* build_item(const item_index& item) {
				...
				if (item.depth == 0) {
					...
					if (item.tree->branch[0] == 0) {
						std::cout << "Tree Being Read" << std::endl; //test
						unsigned long branches[4] = { 0 };
						read_file(branches, item.tree->pos, 4 * sizeof(unsigned long));
						for (int i = 0; i < 4; i++) item.tree->branch[i] = new index_tree(branches[i]);
					}
					...
					std::cout << "Tree Built with branches: " << item_tree->branch[0] << ", " <<
																 item_tree->branch[1] << ", " <<  
																 item_tree->branch[2] << ", " <<  
																 item_tree->branch[3] << std::endl; //test
					return item_tree;
				}
			}
		...
		public:
			...
			bool read(const gmtry2i::vector2i& p, btile<log2_w>* dst) {
				...
				item_index deepest_item = seek_item_at_depth(item_index(indices, map_header.info.depth, map_header.info.origin), p, 0);
			...
			bool read(const gmtry2i::vector2i& p, unsigned int depth, bmap<log2_w>* dst) {
				if (!gmtry2i::contains(get_map_bounds(), p)) return false;
				item_index deepest_item = seek_item_at_depth(item_index(indices, map_header.info.depth, map_header.info.origin), p, depth);
			...
			bool write(const gmtry2i::vector2i& p, const btile<log2_w>* src) {
				fit_map(p);
				write_tile_at_bottom(item_index(indices, map_header.info.depth, map_header.info.origin), p, src);
				return true;
			}
